...
/*   Updated: 2023/02/20 10:18:56 by nakebli          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw2(char c, t_data *data, int *w, int *h)
{
	...
	else if (c == 'E')
	{
		...
		if (!data->image)
			befor_exit(data->map, "\x1B[31mERROR on image box");
		mlx_put_image_to_window(data->mlx, data->win, data->image, *w, *h);
	}
}
...
void	draw(char c, t_data *data, int *w, int *h)
{
	if (c == '1')
	{
		...
		if (!data->image)
			befor_exit(data->map, "\x1B[31mERROR on image tree");
		mlx_put_image_to_window(data->mlx, data->win, data->image, *w, *h);
	}
	else if (c == 'C')
	{
		...
		if (!data->image)
			befor_exit(data->map, "\x1B[31mERROR on image collect");
		mlx_put_image_to_window(data->mlx, data->win, data->image, *w, *h);
	}
}
